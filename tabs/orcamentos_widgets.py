...
class OrcamentosWidgets(ctk.CTkFrame):
    ...
    def FrameOrcamentoEntrys(self):
        self.frames["frame_entrys_orcamentos"] = ctk.CTkFrame(master=self, width=300, height=300, fg_color="gray22")
        self.frames["frame_entrys_orcamentos"].grid(row=0, column=1, padx=5, pady=5, sticky="nwe")
        ...
        for i, (label_text, _) in enumerate(cliente_data_entrys):
            ctk.CTkLabel(master=self.frames["frame_entrys_orcamentos"], text=label_text + ":")\
                .grid(row=i, column=0, padx=10, pady=5, sticky="e")
            entry = ctk.CTkEntry(master=self.frames["frame_entrys_orcamentos"], width=200, placeholder_text=label_text)
            entry.grid(row=i, column=1, padx=10, pady=5, sticky="ew")

        # Última linha para textbox de descrição
        ultima_linha = self.get_ultima_linha(self.frames["frame_entrys_orcamentos"])
        ctk.CTkLabel(master=self.frames["frame_entrys_orcamentos"], text="Descrição:")\
            .grid(row=ultima_linha, column=0, padx=10, pady=5, sticky="ne")
        descricao = ctk.CTkTextbox(master=self.frames["frame_entrys_orcamentos"], width=200, height=100)
